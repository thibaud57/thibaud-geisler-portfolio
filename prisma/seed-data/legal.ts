...
const ProcessingKindSchema = z.enum(["HOSTING", "EMBEDDED_SERVICE", "EMAIL_PROVIDER", "ANALYTICS"])
...
const VatNumberSchema = z.string().regex(/^[A-Z]{2}\d{9,12}$/, "TVA: ISO pays + 9 à 12 chiffres")
...
export const LegalEntitySchema = z
  ...
  .refine((d) => (d.capitalAmount === null) === (d.capitalCurrency === null), {
...
export const dataProcessings: DataProcessingInput[] = [
  ...
  },
  {
    ...
    kind: "EMAIL_PROVIDER",
    purposeFr:
      "Envoi des emails transactionnels du formulaire de contact via serveur SMTP authentifié",
