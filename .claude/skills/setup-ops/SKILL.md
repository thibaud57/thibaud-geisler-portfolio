---
name: setup-ops
description: Bootstrap et diagnostics de l'environnement local. Couvre `just install` (deps pnpm), `just setup` (install + DB readiness + seed), `just check` (diagnostics Node, pnpm, Docker, .env, Postgres). À invoquer au premier clone, après un pull qui change les deps, ou pour diagnostiquer un problème d'env.
...
- `setup` safe à relancer (install idempotent, db readiness idempotent, seed idempotent via `upsert`)
