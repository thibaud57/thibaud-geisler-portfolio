...
- **Curseurs posés une fois en `@layer base`** : preflight v4 retire le curseur des boutons et laisse le curseur texte hériter partout. `globals.css` pose les trois règles : `cursor-default` sur `body`, `cursor: text` sur `input, textarea, [contenteditable]`, `cursor: pointer` sur `button:not(:disabled), [role="button"]:not(:disabled)`. Ajouter `cursor-pointer` dans les CVA des items shadcn cliquables (`DropdownMenuItem`, `SelectItem`, etc.). Ne jamais rattraper un curseur composant par composant, et pas de sélecteur `:is()` étendu. Parti pris : DESIGN.md § Règles
