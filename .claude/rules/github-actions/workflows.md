...
- **Pattern agrégateur** si required check en branch protection avec paths-ignore pour doc-only : split en 3 jobs (`changes` via `dorny/paths-filter@v3` → `quality` conditionnel sur source → `ci` agrégateur qui tourne toujours `if: always()` et retourne success si quality OK ou skipped). Évite que les PR doc-only soient bloquées par le required check. Ajouter `pull-requests: read` aux permissions (paths-filter API). Voir exemple ci-dessous
