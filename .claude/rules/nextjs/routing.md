...
- `generateStaticParams` sur les routes dynamiques `[slug]` reste techniquement **optionnel** avec `cacheComponents: true` (sans lui, rendu à la demande au premier hit puis cache via `'use cache'` + `cacheTag`), mais le projet le pose sur `/projets/[slug]` : le jeu de slugs est fini et la DB est joignable au build (service Postgres éphémère en CI, voir `.claude/rules/nextjs/data-fetching.md`). Bénéfices : premier hit instantané, et métadonnées figées dans le `<head>` pour tous les user-agents, y compris les crawlers HTML-only absents de `htmlLimitedBots` (Telegram, Bluesky, Mastodon). Un slug absent du build reste rendu à la demande, `dynamicParams` ne doit PAS être exporté (build cassé avec `cacheComponents`)
