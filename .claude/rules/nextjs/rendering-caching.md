...
- `connection()` runtime + multi-Suspense + `cacheComponents: true` = bug HierarchyRequestError au reveal côté client sur pages denses (cf. issue [vercel/next.js#86577](https://github.com/vercel/next.js/issues/86577) Open). Préférer le pattern `'use cache'` qui n'introduit pas de zone dynamique. Préférer `io()` (Next 16.3.0) dans les composants : il suspend comme un `await` ordinaire, n'attend pas une vraie navigation et laisse le code aval être mis en cache et préchargé. `connection()` reste pour les route handlers sans arbre React (`sitemap.ts`, `llms.txt`)
