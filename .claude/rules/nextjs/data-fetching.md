...
- Pour les routes dynamiques `/[slug]`, exporter **`generateStaticParams`** qui retourne tous les slugs publics au build → tous prerendered en `◐ Partial Prerender`
...
- Utiliser `unstable_noStore()` : déprécié, remplacé par `connection()` (mais voir gotcha sur `connection()` ci-dessous)
- Utiliser `cache()` de React sur les queries Prisma : redondant avec `'use cache'` (scopes isolés, voir Gotchas). Réserver `cache()` aux fonctions pures appelées plusieurs fois dans le même render sans IO
...
- **Wrapper un Server Component async dans `<Suspense>` quand toutes ses queries sont déjà en `'use cache'`** : redondant et ajoute une frontière inutile dans le shell statique. Voir Gotchas `'use cache'` XOR `<Suspense>`
