...
## À faire
- Configurer `vitest.config.ts` avec le plugin **`@vitejs/plugin-react`** et résoudre les alias `@/*` par la forme native Vitest 4 **`resolve.tsconfigPaths: true`** : le plugin `vite-tsconfig-paths` a été retiré du projet le 2026-09-03, il ne servait plus
...
- Installer les packages canoniques : `vitest`, `@vitejs/plugin-react`, `jsdom`, `@testing-library/react`, `@testing-library/dom`, `@testing-library/jest-dom`, `@testing-library/user-event`
...
## Gotchas
- Vitest 4 : Vite >= 6 + Node.js >= 20 requis (version exacte : `docs/VERSIONS.md`)
...
export default defineConfig({
  plugins: [react()],
  resolve: {
    tsconfigPaths: true, // forme native Vitest 4, aucun plugin d'alias à installer
  },
  test: {
    globals: true,
