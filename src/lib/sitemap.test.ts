...
describe('buildSitemapEntries', () => {
  ...
  it('génère une entrée par projet publié au format /projets/<slug>', () => {
    const entries = buildSitemapEntries({
      staticPaths: [],
      projects: [
        buildProject({ slug: 'digiclaims' }),
        buildProject({ slug: 'foyer' }),
      ],
      siteUrl: SITE_URL_FIXTURE,
    })
    expect(entries).toHaveLength(2)
    expect(entries[0]?.url).toBe('https://thibaud-geisler.com/fr/projets/digiclaims')
  ...
  it('alternates projets exposent fr, en et x-default pointant vers /projets/<slug>', () => {
    const entries = buildSitemapEntries({
      staticPaths: [],
      projects: [buildProject({ slug: 'digiclaims' })],
      siteUrl: SITE_URL_FIXTURE,
    })
    const langs = entries[0]?.alternates?.languages
    expect(langs).toMatchObject({
      fr: 'https://thibaud-geisler.com/fr/projets/digiclaims',
      en: 'https://thibaud-geisler.com/en/projets/digiclaims',
      'x-default': 'https://thibaud-geisler.com/fr/projets/digiclaims',
    })
  })
  ...
  it('ordre : statiques d\'abord, projets ensuite', () => {
    ...
    expect(entries[0]?.url).toBe('https://thibaud-geisler.com/fr/services')
    expect(entries[1]?.url).toBe('https://thibaud-geisler.com/fr/projets/digiclaims')
  })
