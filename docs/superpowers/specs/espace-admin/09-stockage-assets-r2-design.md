...
## Amendement du 2026-09-22 : le logo d'entreprise s'affiche sur la vitrine

La conséquence « le badge entreprise des cartes projet et des case studies n'affiche plus que le nom » est revenue : un projet client montre le logo de l'entreprise qui l'a commandé, comme avant la bascule. Le fichier ne bouge pas, il reste dans `portfolio-admin` sous `freelance/crm/entreprises/<slug>/` : la route publique `/api/assets/[...path]` sert ces seules clés depuis le bucket admin, sans session et en `no-cache` (`isCompanyLogoKey`), à la condition que l'entreprise ait un projet publié (404 sinon, sans lecture de R2), toute autre clé `freelance/` restant derrière `/admin/api/assets` (`isGuardedAssetKey`). La requête publique relit `logoFilename`. Un projet personnel, rattaché à l'entreprise du freelance lui-même, remplace la fiche entreprise par un badge « Projet personnel ». Détail dans ADR-011 et `.claude/rules/nextjs/assets.md`.
