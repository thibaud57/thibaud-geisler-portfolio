...
> **Post-cleanup notice (migration `schema_cleanup` appliquée le 2026-04-29)** : ce spec décrit l'état initial implémenté à cette date. Le schéma a été raffiné ultérieurement par la migration `prisma/migrations/20260429143707_schema_cleanup/`. Changements appliqués depuis :
>
> - **Retiré** : `Publisher.siren` (dérivé via `siret.slice(0, 9)` côté code), `LegalEntity.rcsNumber` (= SIREN dérivable), `Company.locations[]` + enum `CompanyLocation` (granularité couverte par `LegalEntity.address.country` ISO 3166-1 alpha-2 mappé via `Intl.DisplayNames`).
> - **Renommé** : `DataProcessing.legalEntityId` → `processorLegalEntityId` (relation nommée `DataProcessingProcessor` via `processor`) — clarté RGPD controller (Publisher) vs processor (sous-traitant).
> - **Déplacé** : `Project.deliverablesCount` → `ClientMeta.deliverablesCount` (sémantique mission cliente, irrelevant pour PERSONAL).
> - **Ajouté** : enum `DataCategory` (IDENTITY, CONTACT, IP_ADDRESS, USAGE_DATA, TECHNICAL_LOGS, COOKIE_DATA) + `DataProcessing.dataCategories: DataCategory[]` (catégories de données pour le registre des activités de traitement RGPD art. 30 §1 c, obligatoire pour freelance/auto-entrepreneur ; exposé en transparence sur la page `/confidentialite` art. 13 même si l'art. 13 ne le rend pas explicitement obligatoire).
> - **Address.country** : passage de string libre (`"France"`, `"United States"`) à code ISO 3166-1 alpha-2 (`"FR"`, `"US"`, `"LU"`) — résolution UI native via `Intl.DisplayNames(locale, { type: 'region' })`.
> - **Validation Zod runtime active** : `LegalEntitySchema`, `PublisherSchema`, `DataProcessingSchema` désormais appliquées via `parseOrThrow` au seed (regex stricts SIRET/vatNumber/apeCode/country/currency, enum `RegistrationTypeSchema` + `LegalStatusKeySchema`, `.refine()` co-occurrence `(capitalAmount, capitalCurrency)`). Avant : déclarées mais jamais appelées au runtime.
> - **Companies clientes liées à LegalEntity** : 3 `LegalEntity` ajoutées (Foyer S.A. Leudelange LU, CloudSmart S.à r.l. Dudelange LU, Wantedesign SAS Paris FR avec SIRET) + jointure colocalisée via `CompanyInput.legalEntitySlug` direct (au lieu d'un mapping externe). PaySystem reste sans LegalEntity (entité dissoute 2022, infos juridiques publiques indisponibles, TODO commentaire).
> - **`Company.name` Foyer corrigé** : "Foyer Group" → "Foyer" (nom commercial UI ≠ `LegalEntity.name` "Foyer S.A." pour mentions légales).
> - **Helper `parseOrThrow`** : nouveau `prisma/utils.ts` qui factorise le pattern `safeParse + throw` répété au seed.
>
> Les sections ci-dessous décrivent l'état initial. Pour l'état actuel du schéma, source de vérité = `prisma/schema.prisma` + `prisma/seed-data/legal.ts` + `prisma/seed-data/companies.ts`.
