...
- `03-bandeau-consentement-cookies-design.md` (statut: draft) — fournit le `<CookieConsentProvider>` mounté dans `Providers.tsx` qui wrap toute l'arbre, indispensable pour que `<OpenCookiePreferencesButton>` (Client Component descendant) puisse consommer `useConsentStatus().openPreferences()` sans lever d'erreur.
- `04-pages-mentions-confidentialite-design.md` (statut: draft) — fournit `<OpenCookiePreferencesButton>` (Client Component créé au sub 4, étendu au sub 5 avec prop `label?` pour customisation contextuelle, ici on l'utilise avec `variant="link"` + `label={t('Footer.legalNav.cookies')}`) ET le helper `formatSiret()` (créé au sub 4 dans `src/lib/legal/format-siret.ts`, mappe `"88041912200036"` → `"880 419 122 00036"`). Les pages cibles `/mentions-legales` et `/confidentialite` sont également livrées par ce sub 4.
...
**Non touchés** : `src/components/layout/Footer.tsx` row haute (BrandLogo + tagline + location + SocialLinks + DownloadCvButton inchangés), `src/components/features/legal/OpenCookiePreferencesButton.tsx` (consommation tel quel, déjà étendu sub 5), `src/lib/legal/format-siret.ts` (consommation tel quel, livré sub 4), `src/server/queries/legal.ts` (consommation tel quel, livré sub 1), `prisma/schema.prisma` (sub 1), pages `/[locale]/(public)/mentions-legales` et `/confidentialite` (sub 4, déjà existantes), `src/lib/seo/json-ld.ts` (sub 6), `src/components/features/contact/CalendlyWidget.tsx` (sub 5), `next.config.ts` (sub 2), `src/components/providers/Providers.tsx` (sub 3, le `<CookieConsentProvider>` wrap déjà l'arbre).
...
- **`<OpenCookiePreferencesButton variant="link" label={t('Footer.legalNav.cookies')}>` réutilisé du sub 4 + sub 5** : le composant accepte les props `{ className?, variant?, label? }`. Avec `variant="link"`, le bouton shadcn rend visuellement comme un lien texte (pas d'arrière-plan, juste underline au hover), cohérent avec les 2 `<Link>` voisins. Le `label` override le texte par défaut `t('Cookies.banner.customize')` ("Personnaliser") par `t('Footer.legalNav.cookies')` ("Gérer mes cookies" / "Manage my cookies") plus contextuel pour le footer. Voir `.claude/rules/shadcn-ui/components.md` (Button variants, primitive interactive `'use client'`).
...
**GIVEN** la DB seedée par sub 1 (publisher Thibaud avec `siret = "88041912200036"`), build prod via `pnpm build && pnpm start`, et `<CookieConsentProvider>` du sub 3 mounté dans Providers
...
**THEN** la modale Dialog shadcn du sub 3 s'ouvre, affichant les 2 cards (necessary disabled+true, marketing toggle interactive) avec leurs toggles
...
- **`<OpenCookiePreferencesButton>` rendu hors `<CookieConsentProvider>`** : impossible car le Provider du sub 3 wrap toute l'arbre via `Providers.tsx` (qui inclut le Footer). Si un dev futur supprime accidentellement le Provider, `useConsentStatus()` lève l'erreur explicite définie au sub 3, repérable au clic du bouton.
...
**Options envisagées :**
- **A. Réutiliser `<OpenCookiePreferencesButton variant="link" label="Gérer mes cookies">`** : 1 import, 1 ligne de JSX. Le composant gère déjà la consommation de `useConsentStatus().openPreferences()` du sub 3.
...
- **B. `tdd_scope = partial`** avec un test integration `Footer.integration.test.tsx` (mock `useConsentStatus`, mock `Link`, render footer, vérifier les 3 liens + SIRET formaté).
...
- **No-lib-test** appliqué : les règles métier critiques de ce sub-project sont déjà testées indirectement :
  ...
  - `<OpenCookiePreferencesButton>` est testé indirectement au sub 5 (3 cas integration `CalendlyWidget.integration.test.tsx` qui utilisent le hook `useConsentStatus`)
...
- L'option B aurait pour seul mérite de garder la convention "tester chaque sub-project". Mais le coût (mock `useTranslations`, mock `Link`, mock `getPublisher`, mock `useConsentStatus`, render Server Component) est largement supérieur au gain (couverture déjà assurée transversement). Anti-pattern.
