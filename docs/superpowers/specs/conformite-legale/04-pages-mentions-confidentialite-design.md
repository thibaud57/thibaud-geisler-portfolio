...
- `03-bandeau-consentement-cookies-design.md` (statut: draft), fournit le hook `useConsentManager()` (de `@c15t/nextjs`) consommé par `<OpenCookiePreferencesButton>` (section Cookies de `/confidentialite`). Le `<ConsentManagerProvider>` mounté dans `Providers.tsx` au sub 3 wrap déjà l'arbre de toutes les pages publiques, donc le bouton fonctionne sans modification supplémentaire.
...
- **À créer** : `src/components/features/legal/OpenCookiePreferencesButton.tsx` (Client Component `'use client'`, ~20 lignes, consomme `useConsentManager().setActiveUI('dialog')` de `@c15t/nextjs`)
...
- **Bouton client `<OpenCookiePreferencesButton>`** : Client Component leaf marqué `'use client'` qui consomme `useConsentManager()` de `@c15t/nextjs` (mounté par `<ConsentManagerProvider>` au sub 3) et appelle `setActiveUI('dialog')` au clic pour ouvrir la modale `ConsentDialog` c15t. Props minimales `{ className?: string, variant?: 'default' | 'outline' | 'link', label?: string }` pour customisation par caller (sub 4 : variant `outline` dans la section Cookies de `/confidentialite`, sub 7 : variant `link` dans le footer, sub 5 : prop `label` custom "Activer Calendly"). Texte par défaut du bouton via `useTranslations('Cookies').openManagerLabel` ("Gérer mes cookies" / "Manage cookies"). L'import du composant depuis le Server Component de `/confidentialite` est OK (pattern leaf client : un Server Component peut importer et rendre un Client Component, l'inverse casse le RSC). Voir `.claude/rules/nextjs/server-client-components.md` (leaf client component, props sérialisables) et `.claude/rules/shadcn-ui/components.md` (Button shadcn). Voir aussi `docs/knowledges/c15t.md` § "Hook useConsentManager".
...
- **Tableau "Sous-traitants"** : `<table>` HTML standard dans le scope `prose` (Tailwind typography stylise automatiquement les tables). 5 colonnes : Nom, Rôle (`Legal.processingKind.{kind}`), Finalité (`purposeFr` ou `purposeEn` selon locale), Catégories de données (`processing.dataCategories[]` mappées via `t('Legal.dataCategory.{IDENTITY|CONTACT|IP_ADDRESS|USAGE_DATA|TECHNICAL_LOGS|COOKIE_DATA}')` jointes par `, `), Pays (mapping ISO 3166-1 alpha-2 → nom local via `new Intl.DisplayNames([locale], { type: 'region' }).of(address.country)`, ex: `'FR'` → `'France'` en FR / `'United States'` en EN, fallback string brute si code inconnu). Sur mobile (< sm), la table reste responsive grâce au `prose` mais peut overflow horizontal (acceptable MVP, content lisible).
...
**GIVEN** la DB seedée par sub 1 (publisher Thibaud + IONOS hosting + Calendly + IONOS SMTP) et `<ConsentManagerProvider>` du sub 3 mounté dans `Providers.tsx`
...
**AND** la section "Identité de l'éditeur" contient `Thibaud Pierre Geisler`, `Entrepreneur Individuel` (rendu via `t('Legal.legalStatus.entrepreneurIndividuel')`), SIRET formatté `880 419 122 00036`, SIREN `880 419 122` **dérivé via `siret.slice(0, 9)`** (la colonne `Publisher.siren` n'existe plus en BDD post-`schema_cleanup`), `Inscrit au Registre National des Entreprises (RNE) sous le n° SIREN 880 419 122`, code APE `6201Z` avec label `Programmation informatique` (via `t('Legal.ape.6201Z')`), adresse `11 rue Gouvy, 57000 Metz, France` (le pays `France` est dérivé via `Intl.DisplayNames` à partir du code ISO `FR` stocké en BDD), email `contact@thibaud-geisler.com` (clickable `mailto:`), mention `TVA non applicable, art. 293 B du CGI`
**AND** la section "Hébergeur" contient `IONOS SARL`, `SARL au capital de 100 000 EUR` (concaténation côté UI : `t('Legal.legalStatus.sarl')` + ` au capital de ` + `capitalAmount` formaté + ` ` + `capitalCurrency` ; le schéma post-`schema_cleanup` n'a pas de colonne `legalStatusFr/En`, mais `legalStatusKey: String` (i18n) + `capitalAmount Int?` + `capitalCurrency String?`), adresse `7 place de la Gare, 57200 Sarreguemines, France` (pays dérivé via `Intl.DisplayNames`), `RCS Sarreguemines 431 303 775` (le numéro RCS = SIREN dérivé via `siret.slice(0, 9)`, `LegalEntity.rcsNumber` retiré post-`schema_cleanup`), téléphone `+33 9 70 80 89 11`
...
**AND** la section "Hosting provider" affiche IONOS SARL avec concaténation EN `t('Legal.legalStatus.sarl')` ('LLC') + capital formaté → `LLC with capital of 100,000 EUR`
...
**AND** la section "Destinataires / sous-traitants" rend un tableau ou liste structurée avec 3 entrées dans l'ordre : IONOS hosting, Calendly embedded, IONOS SMTP, chacune affichant nom + rôle (Hébergement / Service embarqué / Fournisseur d'emails) + finalité (purposeFr depuis BDD) + **catégories de données** (`processing.dataCategories[]` mappées via `t('Legal.dataCategory.{X}')`, ex IONOS hosting : `Adresse IP, Logs techniques` ; Calendly : `Identité, Contact, Données d'usage, Identifiants cookies` ; IONOS SMTP : `Identité, Contact`) + statut juridique (`t('Legal.legalStatus.{legalStatusKey}')` + capital concaténé côté UI) + pays (dérivé `Intl.DisplayNames` depuis code ISO)
...
- **`publisher.capitalAmount === null`** (cas EI Thibaud, pas de capital) : la section Identité ne rend pas de ligne capital. Pour les sous-traitants société (IONOS = SARL au capital de 100 000 EUR), le capital est rendu côté UI en concaténant `t('Legal.legalStatus.{legalStatusKey}')` + `capitalAmount` formaté + `capitalCurrency` (le schéma post-`schema_cleanup` ne stocke PAS de colonnes `legalStatusFr/En` BDD, le format final est composé au render).
...
- **`<OpenCookiePreferencesButton>` rendu hors `<ConsentManagerProvider>`** : impossible en pratique car le Provider wrap toute l'arbre via `Providers.tsx`. Si le développeur supprime accidentellement le Provider, le hook `useConsentManager()` (de `@c15t/nextjs`) lève une erreur explicite. Pas de gestion défensive supplémentaire.
