...
**AND** la section "Destinataires / sous-traitants" rend un tableau ou liste structurée avec 3 entrées dans l'ordre : IONOS hosting, Calendly embedded, IONOS SMTP, chacune affichant nom + rôle (Hébergement / Service embarqué / Fournisseur d'emails) + finalité (purposeFr depuis BDD) + **catégories de données** (`processing.dataCategories[]` mappées via `t('Legal.dataCategory.{X}')`, ex IONOS hosting : `Adresse IP, Logs techniques` ; Calendly : `Identité, Contact, Données d'usage, Identifiants cookies` ; IONOS SMTP : `Identité, Contact`) + statut juridique (legalStatusFr) + pays (dérivé `Intl.DisplayNames` depuis code ISO)
