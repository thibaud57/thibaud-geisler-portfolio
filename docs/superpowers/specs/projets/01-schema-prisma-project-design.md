...
- **Champ `formats ProjectFormat[]` sur Project** : stocké comme array Postgres natif (Prisma supporte `enum[]`), un projet a 1-N formats (ex: Digiclaims = `[WEB_APP, API]`, Flight Search API = `[API]`).
