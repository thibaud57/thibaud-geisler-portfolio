...
| Sélecteur de date | Popover + Calendar | shadcn/ui | Formulaire projet admin, card Publication : dates de début et de fin. `locale` française passée au `Calendar`, le registry rendant en anglais par défaut |
...
| État vide | Empty | shadcn/ui | Base vide et filtres ou recherche sans résultat d'une liste admin, porté par `DataTable`. L'écran fournit icône, titre et texte de la base vide ; l'état filtré reprend son icône et porte un texte commun à toutes les listes, avec un bouton « Réinitialiser les filtres » (`outline`, `sm`) rendu par `DataTable` lui-même. Bordure pleine (`border-solid`) sur la carte qui l'entoure, pas le `border-dashed` par défaut du registry, comme les autres surfaces admin |
...
- **Badges tronqués** : trois au plus puis un « +N », partout en affichage, là où la maquette en montre deux sur certaines colonnes. Un champ d'édition les montre tous, ils y sont manipulables un par un
...
- **États vides d'une liste admin** : composant `Empty` (shadcn/ui) générique porté par `DataTable`, pour la base vide comme pour les filtres ou la recherche sans résultat, ce dernier avec un bouton « Réinitialiser les filtres » ; l'écran ne fournit que le contenu de la base vide, le texte de l'état filtré étant commun à toutes les listes et seule l'icône y restant celle de l'écran, là où la maquette écrit ce bloc à la main dans chaque écran avec ses propres mots
...
- **Colonne Liens d'un projet** : « GitHub » et « Démo » sont des liens cliquables vers `githubUrl` et `demoUrl`, ouverts dans un nouvel onglet, dans la colonne de la table comme dans la vue détail et la carte mobile, là où la maquette et le spec n'affichent qu'un texte joint signalant leur présence
...
- **Deux champs courts d'une même card** : côte à côte sur une ligne, en `grid-cols-2`, là où la maquette empile parfois. Une card de colonne latérale accueille deux URL ou deux dates sans que l'une ait besoin de toute la largeur, et l'empilement fait grandir la colonne pour rien
...
- **Blocs d'une vue détail** : les mêmes que les cards du formulaire d'édition de l'élément, sous les mêmes titres, dans le même ordre et avec les mêmes champs, là où la maquette ne montre qu'une grille plate. Consulter puis modifier devient un seul geste : ce qu'on vient de lire se retrouve là où on va l'éditer. Les titres viennent d'une constante que le formulaire et le détail partagent, jamais de deux chaînes recopiées. Un champ dont la valeur se compte en milliers de caractères, comme un case study, s'y résume à sa présence et à sa taille : le lire relève de l'écran d'édition
- **Sous-titre d'une vue détail** : ce qui situe l'élément d'un coup d'œil, et cela seulement. Y vont son rang, sa nature et son état (rang d'affichage et statut d'un projet, engagement et décompte de projets d'une entreprise), qu'ils aient une card d'édition ou non ; le champ quitte alors le bloc de sa card plutôt que d'y être répété. Une valeur calculée depuis les champs d'une card, elle, reste dans le bloc de cette card, comme la durée sous les deux dates dont elle dérive
