...
> Chaque entrée rejoint sa famille ci-dessus au moment de son installation. Ce qui est post-MVP ici est l'**usage**, pas toujours le composant : `Badge`, `Command`, `Dialog`, `Input`, `Popover`, `Tabs` et `Tooltip` tournent en production, les écrans qui les emploieront de cette façon n'existent pas.
