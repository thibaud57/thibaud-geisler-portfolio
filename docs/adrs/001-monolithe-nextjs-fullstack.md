...
La séparation logique est maintenue via le route group `(public)/` sous `[locale]/`, le segment `admin/` à la racine de `app/` (voir [ADR-022](022-routing-espace-admin.md)) et la structure `src/server/`. Cette architecture peut évoluer sans refonte si un backend séparé devenait nécessaire.
